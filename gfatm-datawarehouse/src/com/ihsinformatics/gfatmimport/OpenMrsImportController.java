...
public class OpenMrsImportController extends AbstractImportController {
	...
	public OpenMrsImportController(DatabaseUtil sourceDb, DatabaseUtil targetDb, Date fromDate, Date toDate) {
		this.sourceDb = sourceDb;
		this.targetDb = targetDb;
	...
	private void clearTempTables(int implementationId) {
		String[] tables = { "tmp_person", "tmp_person_attribute", "tmp_person_attribute_type", "tmp_person_address",
				"tmp_person_name", "tmp_role", "tmp_role_role", "tmp_privilege", "tmp_role_privilege", "tmp_users",
				"tmp_user_property", "tmp_user_role", "tmp_provider_attribute_type", "tmp_provider",
				"tmp_provider_attribute", "tmp_location_attribute_type", "tmp_location", "tmp_location_attribute",
				"tmp_location_tag", "tmp_location_tag_map", "tmp_concept_class", "tmp_concept_set",
	...
	public void importUserData(DatabaseUtil remoteDb, int implementationId)
		...
		try {
			...
			selectQuery = "SELECT 0,'" + implementationId + "', role, description, uuid FROM " + database + "."
					+ tableName + " AS t ";
			log.info("Inserting data from " + database + "." + tableName + " into data warehouse");
			remoteSelectInsert(selectQuery, insertQuery, remoteDb.getConnection(), targetDb.getConnection());
			insertQuery = "INSERT IGNORE INTO " + tableName + " SELECT DISTINCT * FROM tmp_" + tableName
			...
			selectQuery = "SELECT 0,'" + implementationId + "', parent_role, child_role FROM " + database + "."
					+ tableName + " AS t ";
			log.info("Inserting data from " + database + "." + tableName + " into data warehouse");
			remoteSelectInsert(selectQuery, insertQuery, remoteDb.getConnection(), targetDb.getConnection());
			...
			selectQuery = "SELECT 0,'" + implementationId + "', privilege, description, uuid FROM " + database + "."
					+ tableName + "  AS t ";
			log.info("Inserting data from " + database + "." + tableName + "  into data warehouse");
			remoteSelectInsert(selectQuery, insertQuery, remoteDb.getConnection(), targetDb.getConnection());
			insertQuery = "INSERT IGNORE INTO " + tableName + " SELECT DISTINCT * FROM tmp_" + tableName
			...
			selectQuery = "SELECT 0,'" + implementationId + "', role, privilege FROM " + database + "." + tableName
					+ " AS t ";
			log.info("Inserting data from " + database + "." + tableName + " into data warehouse");
			remoteSelectInsert(selectQuery, insertQuery, remoteDb.getConnection(), targetDb.getConnection());
			deleteQuery = "DELETE FROM " + tableName + " WHERE implementation_id = '" + implementationId + "'";
			...
			selectQuery = "SELECT 0,'" + implementationId + "', user_id, property, property_value FROM " + database
					+ "." + tableName + " AS t ";
			log.info("Inserting data from " + database + "." + tableName + " into data warehouse");
			remoteSelectInsert(selectQuery, insertQuery, remoteDb.getConnection(), targetDb.getConnection());
			deleteQuery = "DELETE FROM " + tableName + " WHERE implementation_id = '" + implementationId + "'";
			...
			selectQuery = "SELECT 0,'" + implementationId + "', user_id, role FROM " + database + "." + tableName
					+ " AS t ";
			log.info("Inserting data from " + database + "." + tableName + " into data warehouse");
			remoteSelectInsert(selectQuery, insertQuery, remoteDb.getConnection(), targetDb.getConnection());
			deleteQuery = "DELETE FROM " + tableName + " WHERE implementation_id = '" + implementationId + "'";
	...
	public void importLocationData(DatabaseUtil remoteDb, int implementationId)
		...
		try {
			...
			selectQuery = "SELECT 0,'" + implementationId + "', location_id, location_tag_id FROM " + database + "."
					+ tableName + " AS t ";
	...
	public void importEncounterData(DatabaseUtil remoteDb, int implementationId)
		...
		try {
			...
			insertQuery = "INSERT INTO tmp_" + tableName
					+ " (surrogate_id, implementation_id, encounter_type_id, name, description, creator, date_created, retired, retired_by, date_retired, retire_reason, uuid, edit_privilege, view_privilege, date_changed, changed_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			selectQuery = "SELECT 0,'" + implementationId
					+ "', encounter_type_id, name, description, creator, date_created, retired, retired_by, date_retired, retire_reason, uuid, edit_privilege, view_privilege, date_changed, changed_by FROM "
					+ database + "." + tableName + " AS t " + filter("t.date_created", null);
			...
			// Encounter
			tableName = "encounter";
			Object[][] createdDates = remoteDb.getTableData(tableName, "DATE(date_created)",
					"WHERE date_created BETWEEN TIMESTAMP('" + DateTimeUtil.toSqlDateTimeString(fromDate)
							+ "') AND TIMESTAMP('" + DateTimeUtil.toSqlDateTimeString(toDate) + "')",
					true);
			Object[][] updatedDates = remoteDb.getTableData(tableName, "DATE(date_changed)",
					"WHERE date_changed BETWEEN TIMESTAMP('" + DateTimeUtil.toSqlDateTimeString(fromDate)
							+ "') AND TIMESTAMP('" + DateTimeUtil.toSqlDateTimeString(toDate) + "')",
					true);
			...
			createdDates = remoteDb.getTableData(tableName, "DATE(date_created)", filter("date_created", null), true);
			dates = new ArrayList<>();
			for (Object[] date : createdDates) {
				dates.add(date[0].toString());
			}
			...
			createdDates = remoteDb.getTableData(tableName, "DATE(date_created)", filter("date_created", null), true);
			Object[][] voidedDates = remoteDb.getTableData(tableName, "DATE(date_voided)", filter("date_voided", null),
					true);
			dates = new ArrayList<>();
			for (Object[] date : createdDates) {
				dates.add(date[0].toString());
	...
	public void importFormData(DatabaseUtil remoteDb, int implementationId)
		...
		try {
			...
			selectQuery = "SELECT 0,'" + implementationId + "',field_id ,answer_id ,creator, date_created,uuid FROM "
					+ database + "." + tableName + " AS t " + filter("t.date_created", null);
			log.info("Inserting data from " + database + ".encounter_type into data warehouse");
			remoteSelectInsert(selectQuery, insertQuery, remoteDb.getConnection(), targetDb.getConnection());
			insertQuery = "INSERT IGNORE INTO " + tableName + " SELECT DISTINCT * FROM tmp_" + tableName
			...
			selectQuery = "SELECT 0,'" + implementationId
					+ "',field_type_id ,name ,description ,is_set ,creator ,date_created ,uuid FROM " + database + "."
